...
const DesignProcessForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchDesignProcess = async () => {
      try {
        ...
        const { title, subheading, description, hours, priority, image: currentImage, alt } = response.data.data;
        setTitle(title);
        setSubheading(subheading);
        setDescription(description);
        setHours(hours);
        setPriority(priority);
        setCurrentImage(currentImage); // Set the current image URL
        setPhotoAlt(alt);
      } catch (error) {
        console.error(error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (image) {
        formData.append('images', image); // Add new image
